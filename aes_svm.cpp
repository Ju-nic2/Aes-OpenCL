...
int main(int argc, char **argv)
{
	...
	struct timeval kernelCreateStart, kernelCreateEnd;
	struct timeval kernelArgStart, kernelArgEnd;
	...
	gettimeofday(&memEnd,NULL);
		...
		//printf("3%s\n",TranslateOpenCLError(err));
		 err = clSetKernelArgSVMPointer(kernel, 0, (void *)data);
		...
		gettimeofday(&memStart,NULL);
		err = clEnqueueSVMMap (cmdqueue, CL_TRUE, CL_MAP_READ, data, sizeof(unsigned char)*16*maxCoreNum, 0, 0, 0);
		gettimeofday(&memEnd,NULL);
		memTime += (double)((memEnd.tv_sec - memStart.tv_sec) * 1000 + (double)(memEnd.tv_usec - memStart.tv_usec) /1000);
